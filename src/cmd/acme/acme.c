...
iconinit(void)
{
	...
	if(tagcols[BACK] == nil) {
		/* Blue */
		tagcols[BACK] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x151210FF);
		tagcols[HIGH] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0xc4c5b5FF);
		tagcols[BORD] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x9d65ffFF);
		tagcols[TEXT] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0xc4c5b5FF);
		tagcols[HTEXT] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x222222FF);

		/* Yellow */
		textcols[BACK] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x0c0c0cFF);
		textcols[HIGH] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x8a776aFF);
		textcols[BORD] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x625e4cFF);
		textcols[TEXT] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0xd9d9d9FF);
		textcols[HTEXT] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x222222FF);
	}
	...
	tmp = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x9d65ffFF);
	draw(modbutton, r, tmp, nil, ZP);
	freeimage(tmp);

	r = button->r;
	colbutton = allocimage(display, r, RGBA32, 1, 0xf4005fFF);
